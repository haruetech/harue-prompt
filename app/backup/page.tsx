...
const PRESETS: Record<ItemKey, Preset> = {
  "apparel:outer": {
    ...
    shot: {
      thumbnail:
        "full body model, straight standing pose, outfit centered, soft shadow",
      detail:
        "close-up, focus on fabric weave, stitching, buttons or zipper, collar detail",
      lookbook:
        "editorial lookbook, cinematic lighting, minimal set, elegant styling, luxury brand atmosphere",
    },
  },
  ...
  "apparel:tshirt": {
    ...
    shot: {
      thumbnail:
        "full body model, straight standing pose, clean framing, soft shadow",
      detail:
        "close-up, focus on neckline rib, sleeve hem, stitching, fabric texture",
      lookbook:
        "casual lookbook, natural daylight, minimal styling, lifestyle mood, premium casual",
    },
  },
  ...
  "apparel:pants": {
    ...
    shot: {
      thumbnail:
        "full body model, legs visible, natural standing pose, silhouette clear",
      detail:
        "close-up, focus on waistband, zipper, pockets, stitching, hem, fabric texture",
      lookbook:
        "editorial lookbook, walking pose, natural motion, luxury mood, cinematic light",
    },
  },
  ...
  "apparel:skirt": {
    ...
    shot: {
      thumbnail:
        "full body model, skirt length visible, centered composition, soft shadow",
      detail:
        "close-up, focus on pleats, waistband, zipper, lining detail, fabric texture",
      lookbook:
        "editorial lookbook, gentle motion, airy mood, premium styling, cinematic lighting",
    },
  },
  ...
  "apparel:dress": {
    ...
    shot: {
      thumbnail:
        "full body model, dress fully visible, centered composition, soft shadow",
      detail:
        "close-up, focus on neckline, waist seam, zipper or buttons, fabric texture",
      lookbook:
        "luxury lookbook, cinematic lighting, refined pose, minimal props, editorial mood",
    },
  },
  ...
  "apparel:knit": {
    ...
    shot: {
      ...
      detail:
        "macro close-up, focus on knit weave, rib cuffs, button detail if cardigan",
      lookbook:
        "editorial cozy lookbook, soft cinematic light, minimal set, premium styling",
    },
  },
  ...
  "shoes:sneakers": {
    ...
    shot: {
      thumbnail:
        "female model wearing sneakers, full body or 3/4 body, natural standing pose, focus on shoes",
      detail:
        "close-up, focus on toe box, laces, stitching, outsole texture, sharp details",
      lookbook:
        "editorial lookbook, walking motion, modern minimal styling, cinematic lighting, focus on footwear",
    },
  },
  ...
  "shoes:loafers": {
    ...
    shot: {
      thumbnail:
        "female model wearing loafers, full body framing, ankle visible, focus on shoes",
      detail:
        "close-up, focus on leather grain, stitching, vamp detail, outsole edge",
      lookbook:
        "high-end lookbook, refined styling, minimal set, cinematic lighting, focus on loafers",
    },
  },
  ...
  "shoes:flats": {
    ...
    shot: {
      thumbnail:
        "female model wearing flats, full body framing, natural standing pose, focus on shoes",
      detail: "close-up, focus on toe shape, piping, insole texture, outsole",
      lookbook:
        "editorial lookbook, graceful pose, minimal styling, cinematic light, focus on flats",
    },
  },
  ...
  "shoes:pumps": {
    ...
    shot: {
      thumbnail:
        "female model wearing pumps, full body framing, elegant stance, focus on heel and toe",
      detail:
        "close-up, focus on heel construction, insole, stitching, toe box",
      lookbook:
        "luxury editorial lookbook, refined pose, minimal set, cinematic lighting, focus on pumps",
    },
  },
  ...
  "shoes:sandals": {
    ...
    shot: {
      thumbnail:
        "female model wearing sandals, full body framing, relaxed pose, focus on straps and sole",
      detail: "close-up, focus on strap texture, buckle, stitching, outsole",
      lookbook:
        "summer lookbook, airy mood, natural lighting, minimal styling, focus on sandals",
    },
  },
  ...
  "shoes:mules": {
    ...
    shot: {
      thumbnail:
        "female model wearing mules, full body framing, casual standing pose, focus on shoes",
      detail: "close-up, focus on upper shape, insole, stitching, outsole",
      lookbook:
        "editorial lookbook, minimal set, relaxed luxury mood, cinematic lighting, focus on mules",
    },
  },
  ...
  "shoes:slippers": {
    ...
    shot: {
      thumbnail:
        "female model wearing slippers, full body framing, indoor cozy mood, focus on shoes",
      detail: "close-up, focus on upper material, stitching, footbed, outsole",
      lookbook:
        "cozy lifestyle lookbook, soft daylight, minimal set, premium comfort mood, focus on slippers",
    },
  },
  ...
  "shoes:boots": {
    ...
    shot: {
      thumbnail:
        "female model wearing boots, full body framing, focus on boots silhouette",
      detail: "close-up, focus on zipper, stitching, leather grain, outsole",
      lookbook:
        "winter lookbook, cinematic lighting, luxury styling, minimal set, focus on boots",
    },
  },
  ...
  "shoes:dress_shoes": {
    ...
    shot: {
      thumbnail:
        "female model wearing dress shoes, full body framing, clean formal styling, focus on silhouette",
      detail:
        "close-up, focus on toe shape, stitching, leather grain, heel/outsole construction",
      lookbook:
        "formal editorial lookbook, refined pose, minimal set, cinematic lighting, focus on dress shoes",
    },
  },
};
...
function negativeBoostByShot(shot: Shot): string[] {
  if (shot === "detail") {
    return ["full body", "wide shot", "too much background"];
  }
  if (shot === "thumbnail") {
    return ["awkward framing", "cut off feet", "cut off head"];
  }
...
function buildPromptGuideKorean(params: {
  ...
  const { group, shot, itemLabel, bgText, color, material, heel, extra } = params;
...
function buildNegativeGuideKorean(params: { bgCategory: BgCategory; shot: Shot }) {
  const { bgCategory, shot } = params;
  const lines: string[] = [];
  ...
  return (
    <button
      ...
      style={{  
        ...
        background: active ? "#3A3A3A" : "#fff",
        color: active ? "#fff" : "#111",
        border: active ? "1px solid #3A3A3A" : "1px solid #ddd",
        cursor: "pointer",
        fontWeight: 900,
        letterSpacing: -0.2,
      }}
...
export default function Page() {
  ...
  const prompt = useMemo(() => {
    const bodyLock = shot !== "detail" ? FULL_BODY_LOCK : "";
  ...
  }, [preset, shot, backgroundPhrase, color, material, heel, extra, group]);

  // ✅ Negative Prompt (자동 보강: 블러 + 크롭차단 + 배경/샷별 보강)
  ...
  const promptGuideKorean = useMemo(
    ...
    [group, shot, itemLabel, bgText, color, material, heel, extra]
  );

  const negativeGuideKorean = useMemo(
    () => buildNegativeGuideKorean({ bgCategory, shot }),
    [bgCategory, shot]
  );
  ...
  return (
    <main style={{ maxWidth: 1120, margin: "0 auto", padding: 26 }}>
          ...
          <div style={{ border: "1px solid #eee", borderRadius: 18, padding: 14, marginBottom: 14, background: "#fff" }}>
            ...
            <Panel title="배경 커스텀(직접 입력)">
              <input
                ...
                style={{width: "100%",  boxSizing: "border-box",  padding: "8px 12px",  borderRadius: 12,  border: "1px solid #ddd",}}
              />
           ...
           >
            <Panel title="색상">
              <input
                ...
                style={{ width: "100%", padding: "8px 12px", borderRadius: 12, border: "1px solid #ddd" }}
              />
            </Panel>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 0.98fr", gap: 14, marginTop: 16 }}>
        ...
        <div style={{ border: "1px solid #ddd", borderRadius: 18, padding: 14, background: "#fff" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
            <strong>⛔ Negative Prompt (자동 보강)</strong>
            <button
